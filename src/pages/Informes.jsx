...
export default function Informes() {
  ...
  const [generando, setGenerando] = useState({ nota: false, notaDocx: false, planilla: false })
  ...
  const generarNotaWord = async () => {
    if (!numeroNota) { setError('Ingresá el número de nota antes de generar'); return }
    setGenerando(g => ({ ...g, notaDocx: true }))
    setError('')
    try {
      const expedientesFiltrados = Object.values(ordenGruposInforme).flat()
      const { data } = await axios.post('/api/informes/nota-docx', {
        mes, anio, numeroNota,
        expedientes: expedientesFiltrados,
      })
      setArchivos(prev => [...prev, { tipo: 'Word Nota', url: data.url, nombre: data.archivo }])
      window.open(data.url, '_blank')
    } catch (err) {
      setError(err.response?.data?.error || 'Error al generar la nota en Word')
    } finally { setGenerando(g => ({ ...g, notaDocx: false })) }
  }
        ...
        <>
          ...
          <div className="card">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                onClick={generarNotaWord}
                disabled={generando.notaDocx || !preview.totalOPs}
                className="btn-primary flex items-center gap-2"
